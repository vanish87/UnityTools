...
namespace UnityTools.Common
{
	...
	{
		static public Texture2D GenerateGradientTexture(List<AnimationCurve> from, int size = 128)
		{
			var w = size;
			var h = from.Count;

			var texture = TextureManager.Create(w, h, TextureFormat.RFloat, false, true);
		...
		static public void UpdateGradientTexture(Texture2D tex, List<AnimationCurve> from, int size = 128)
		{
			var w = size;
			var h = from.Count;
