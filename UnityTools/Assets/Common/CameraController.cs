...
namespace UnityTools.Common
{
	public class CameraController : MonoBehaviour
	{
		...
		{
			if (Input.GetKeyDown(KeyCode.F10)) this.controllable = !this.controllable;
		...
		protected void Zoom()
		{
			...
			if (moveAmount != 0)
			{
				...
				if (this.limitZoomX)
				{
					this.moveTarget.x = target.transform.position.x;
				}

				if (this.limitZoomY)
				{
					this.moveTarget.y = target.transform.position.y;
				}

				if (this.limitZoomZ)
				{
					this.moveTarget.z = target.transform.position.z;
				}
			}

			if (this.smoothZoom)
			{
				if (this.moveTarget == target.transform.position)
				{
					this.moveTarget = target.transform.position;
				}
		...
		protected void Rotate()
		{
			...
			if (Input.GetMouseButton((int)this.rotateTrigger))
			{
				...
				{
					this.rotateTarget = Quaternion.AngleAxis(mouseY * this.rotateSpeed, Vector3.Cross(target.transform.forward, Vector3.up)) * this.rotateTarget;
				}
			}
		...
		protected void Pan()
		{
			...
			if (Input.GetMouseButton((int)this.dragTrigger))
			{
				...
				if (!this.limitPanX)
				{
					this.moveTarget += target.transform.right * mouseX * dragSpeed;
				}

				if (!this.limitPanY)
				{
					this.moveTarget += target.transform.up * mouseY * dragSpeed;
				}
			}
